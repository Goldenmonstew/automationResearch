...
1. **drafting**：`len(draft_nodes) < num_drafts`（默认 3，`bfts_config.yaml:76`）→ append `None`（让 worker 新建一个 draft 根）。
...
- 通用上限：`len(journal.nodes) >= stage.max_iterations`（stage1=20, stage2=12, stage3=12, stage4=18，`bfts_config.yaml:40-43`）。
...
- **进程池 / GPU 分配**：`ProcessPoolExecutor(max_workers=num_workers)`（默认 4，`bfts_config.yaml:38`）。检测到 GPU（nvidia-smi，`parallel_agent.py:1120-1139`）则 `num_workers=min(num_workers, num_gpus)`，`GPUManager` 给每 worker 独占一卡（`CUDA_VISIBLE_DEVICES`，`:1443-1448`）；无 GPU 则 CPU-only。
...
**一个硬性工程约束（论文未点明）**：`backend_anthropic.py:33-36` 直接 `NotImplementedError` function calling。因此所有需结构化输出的环节（is_bug 判定、VLM 分析、metric 解析、best-node 选择、stage 完成评估）**必须走 OpenAI 系模型**，Claude 只负责自由文本的 plan+code 生成。o1 在 backend 被特判 `reasoning_effort=high` + `max_completion_tokens=100000`（`backend/__init__.py:62-63`）。

**模型版本说明**：本仓库快照早于论文定稿的模型矩阵（Sonnet 4 / o4-mini 尚未出现），由 3.5-Sonnet / o3-mini / o1 / gpt-4o 占位，但**角色分工与论文一致**。应以查询到的实际配置值为准。

**成本**：源码未直接给出每篇论文美元成本（论文正文给的 ~$15-20/paper 量级数字不在本次源码范围内）。可观测的成本控制点：单实验 1h 上限、各 stage max_iters（20/12/12/18）、`num_workers=4`、`save_token_tracker`（launch `:269`）统计 token。
...
```
- **关键配置 `bfts_config.yaml`**：`num_workers:4`（`:38`）、`exec.timeout:3600`（每节点 1h，`:19`）、各 stage `max_iters`（20/12/12/18，`:40-43`）、`search.num_drafts:3`（`:76`）/`debug_prob:0.5`（`:75`）/`max_debug_depth:3`（`:74`）、`num_seeds:3`（`:48`）、角色模型 `agent.code/feedback/vlm_feedback`（`:57,63,69`）。launch 会把它拷成 per-idea config 并注入路径。
