...
> **本仓库不含官方代码与论文 PDF。** 三个官方 clone(`AI-Scientist` / `AI-Scientist-v2` / ICLR2025 workshop)各有自己的 git 与公开仓库,Nature 论文 PDF 有版权,均按 `.gitignore` 排除。对 v2 的适配改动以 diff 形式存于 `patches/`(用 `git apply` 打到官方 clone 上)。
...
| 想了解 | 读这个 | 是什么 |
|---|---|---|
| 系统怎么运作(源码级) | `STUDY_NOTES.md` | 28k 字,论文术语 ↔ 三仓库 `文件:行号` 逐一对照 |
| 方法论拆解 | `METHODOLOGY_ANALYSIS.md` | 想法算力分级 / 树搜索决策机制 / token / 三层超时 / 跨平台 |
| 一次完整复现长什么样 | `EXPERIMENT_REPORT.md` | 从"看论文找代码"到自有 GPU 跑通 v2、产出一篇 AI 自主完成的 ML 论文 |
| 不同 LLM 当"科学家"差多少 | `BENCHMARK_REPORT.md` | v1/v2 × 多模型三层 benchmark(写作 / 树搜索 / 想法生成)+ 质量 gap 归因 |
| 这套东西能不能用、值多少 | `FEASIBILITY_REPORT.md` | 资源账本、照搬可行性、能力边界、论文可用性(含放大配置档试点) |
| 评测协议为何这样设计 | `PREREGISTRATION.md` · `COMPREHENSIVE_PROTOCOL.md` | 数据产出前冻结的预注册:选择规则、声明层级、审计校准承诺 |
...
- **外挂一个"零造假 gate"代价很小、收益是质变。** 经 gate 认证的诚实版相对原始版的"诚实税"均值仅 **−0.28**(28 个实例里 8 个零税),过 gate 后每条结论都能逐条溯源到日志。
...
> 完整数字、置信区间、威胁有效性的讨论见 `FEASIBILITY_REPORT.md` 与 `BENCHMARK_REPORT.md`。

---

## 两套被研究的系统

官方系统有两代,架构差异很大,复现路径也不同(两者均为官方 clone,不在本仓库,适配 diff 见 `patches/`):
...
`tools/` 是这个工作区**额外**搭的脚手架,不属于官方系统,包含:一键复现(`setup_ai_scientist.sh`)、token 计量、想法多评委 ensemble 选择(`select_ideas` / `merge_selection` / `recurate_8h`)、grounding 审计与定点改写、5 票 ensemble 评审、成对盲评锦标赛、审计器校准、批量编排与抢救链。

---

## 复现:快速开始

> **前置:** 一个 OpenAI 兼容的 LLM 端点(官方 OpenAI key 或任意兼容网关,key 经环境变量注入、不写进代码)+ 一块能跑 PyTorch+CUDA 的 NVIDIA GPU。树搜索/训练只认 CUDA,Apple Silicon (MPS) 不支持——本机只能跑纯 API 的想法生成。
...
> 用非官方端点时需要的适配清单(模型名路由规范化、引用搜索改用 OpenAlex、reasoning 模型输出解析鲁棒化、LaTeX 工具链用户态安装等)与动机见 `FEASIBILITY_REPORT.md` §2–3 与 `STUDY_NOTES.md`;其中若干工程缺陷已整理为上游 issue。
...
4. **预注册** — 选择规则、声明层级、审计承诺在数据产出前冻结(`PREREGISTRATION.md`),避免事后挑数据。
...
- **结论边界:** 全部评测由 LLM 评委完成,与人类同行评审决策之间的效度鸿沟未闭合;实验域限于"代码即实验"的小数据 / 小模型 / 训练动力学诊断(T0–T1),更大规模(T2–T3)未跑通。详见 `FEASIBILITY_REPORT.md` §6–7。
