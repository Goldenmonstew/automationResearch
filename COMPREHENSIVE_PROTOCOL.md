...
>
> - **G1 超过官方(同标尺)**:在**同一套评审仪器**下不弱于/优于官方 showcase。注意:修正(2026-06-11):官方 6.33 是人类 workshop 尺,与任何 AI 评审尺不可比;G1 的合法形态 = 绝对尺(5 票 ensemble)宣称同档 + **成对盲评锦标赛胜率 + 置信区间**宣称优越(见 PREREGISTRATION.md)。最终结果:胜率 84.4% [0.756, 0.905]。
...
- 出对照表:
  - **G1**(2026-06-11 修正口径):绝对尺只报"同档";优越性 = 成对盲评胜率 + CI(实测 84.4% [0.756, 0.905],逐篇 CI 下界全 >50%)。6.33 为人类尺,不进任何对比。⚠️ CI 聚类警示:90 场配对并非独立二项——对手仅 n=3 篇、每对重复 6 次,匹配高度聚类,二项/Wilson CI 低估方差,有效样本接近 pairs n=15;因此该头对头胜率 CI 应视为乐观,保守下界按 n=15 重算约 0.62。
