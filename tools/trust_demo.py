#!/usr/bin/env python
"""Demo: AI Research Trust Verifier on 4 real-world failure modes.
  (1) fabricated datasets/baselines  (2) honest  (3) degenerate/saturated
  (4) subtle overstatement (no significance test)
...
SAMPLES = [
 {"name": "① 造假(编造数据集/baseline)", "expect": "NOT TRUSTWORTHY",
  ...
  "metrics": {}},
 {"name": "② 诚实(声明=证据)", "expect": "TRUSTWORTHY",
  ...
  "metrics": {"acc_finals": [0.765, 0.52]}},
 {"name": "③ 退化(100%饱和当成果)", "expect": "NOT TRUSTWORTHY",
  ...
  "metrics": {"acc_finals": [1.0, 1.0, 0.999, 1.0]}},
 {"name": "④ 夸大(无显著性)", "expect": "QUESTIONABLE",
  ...
  "metrics": {"acc_finals": [0.4658, 0.3288]}},
]
...
def verify_retry(s, tries=3):
    ...
    for _ in range(tries):
        r = verify(s)
        if r["grounding"]["verdict"] not in ("ERROR", "UNCLEAR"):
            return r
        time.sleep(5)
    return r
